...
//patch: add --seek-utc0 N (align UTC_START to DF=0 frame)  ver=2026.02.12-01
// 运行示例：
...
//       --dfmax 31249 --batch 64 --stat 1.0 --seek-utc0 2
...
// patch version (printed by --help)
#define UDP2DADA_VDIF_BLK2_PATCHVER "2026.02.12-01"
...
/* 解析 VDIF 头：
   ...
   ref_epoch = w1[29:24]
   data_frame= w1[23:0]
   le: 1=小端, 0=大端。  :contentReference[oaicite:1]{index=1} */
...
int main(int argc,char**argv)
{
  ...
  int seek_utc0 = 2;              // 若首帧 df!=0，向后寻找 df=0 的整秒帧窗口(秒)。0=禁用
  ...
  for(int i=1;i<argc;i++)
  {
    if(!strcmp(argv[i],"-k")&&i+1<argc) key_str=argv[++i];
    ...
    } else if(!strcmp(argv[i],"--seek-utc0")&&i+1<argc){ int v=atoi(argv[++i]); if(v<0) v=0; seek_utc0=v;
      ...
      fprintf(stderr,"Usage: %s -k a000 --header header_all.cfg [--ip IP --port 17200]\n"
                     ...
                     "       [--seek-utc0 N]  # 若首帧 df!=0，向后 N 秒内寻找 df=0 整秒帧作为 UTC_START (0=禁用, 默认 2)\n"
                     "PatchVer: %s\n", argv[0], UDP2DADA_VDIF_BLK2_PATCHVER);
      return 0;
    }
  }
  ...
  fprintf(stderr,"Listen %s:%d  (VDIF %s, payload %s, DF_MAX=%u, batch=%d, stat=%.3fs, seek-utc0=%d)\n",
          ip,port, vdif_le?"LE":"BE", payload_be?"BE":"LE", DF_MAX, BATCH, stat_interval, seek_utc0);

  // 先收一帧确定 UTC_START（可选：若 df!=0，向后寻找 df=0 的整秒帧）
  ...
  // 先打印收到的第一帧（未对齐）
  char utc0[64]; utc_str(abs_sec,utc0,sizeof(utc0));
  fprintf(stderr,"First VDIF(recv): ref=%d sec=%u df=%u -> UTC=%s\n",ref_epoch,sec,df,utc0);

  // 若首包 df!=0：在后续 seek_utc0 秒内寻找 df=0 帧；找到则以该整秒帧作为真正的第一帧 & UTC_START。
  if(df != 0 && seek_utc0 > 0)
  {
    const int64_t SEEK_MAX_SEC = (int64_t)seek_utc0;
    uint8_t tmp[FRAME_SIZE];
    uint64_t dropped = 0;
    int found = 0;
    time_t abs_first = abs_sec;
    uint32_t df_first = df;

    while(!stop_flag)
    {
      ssize_t nn = recvfrom(s,tmp,sizeof(tmp),0,NULL,NULL);
      if(nn!=FRAME_SIZE){ fprintf(stderr,"seek recv %zd\n",nn); break; }
      dropped++;

      int ref2=0; uint32_t sec2=0, df2=0;
      parse_vdif_hdr(tmp,vdif_le,&ref2,&sec2,&df2);
      time_t abs2 = vdif_epoch_to_time_t(ref2,sec2);

      int64_t dsec = (int64_t)abs2 - (int64_t)abs_first;
      if(dsec < 0)
        continue;  // 乱序/回退：继续等待

      if(df2 == 0)
      {
        // 找到整秒起点：用该帧作为输出第一帧，同时 UTC_START 也写该整秒
        memcpy(first,tmp,FRAME_SIZE);
        ref_epoch = ref2; sec = sec2; df = df2; abs_sec = abs2;
        found = 1;
        break;
      }

      if(dsec > SEEK_MAX_SEC)
      {
        // 超出窗口仍未遇到 df=0：不再继续丢弃，直接从当前帧开始（可能不是整秒）
        memcpy(first,tmp,FRAME_SIZE);
        ref_epoch = ref2; sec = sec2; df = df2; abs_sec = abs2;
        found = 0;
        break;
    ...
    char utc1[64]; utc_str(abs_sec,utc1,sizeof(utc1));
    if(found)
      fprintf(stderr,"[SEEK_UTC0] first df=%u, found DF=0 within %d s, dropped=%llu -> UTC_START=%s\n",
              df_first, seek_utc0, (unsigned long long)dropped, utc1);
    else
      fprintf(stderr,"[WARN][SEEK_UTC0] first df=%u, DF=0 not seen within %d s, start at df=%u (dropped=%llu) -> UTC_START=%s\n",
              df_first, seek_utc0, df, (unsigned long long)dropped, utc1);
  }

  // 最终用于写入 DADA 头的 UTC_START
  char utc[64]; utc_str(abs_sec,utc,sizeof(utc));

  fprintf(stderr,"First VDIF(use):  ref=%d sec=%u df=%u -> UTC_START=%s\n",ref_epoch,sec,df,utc);
